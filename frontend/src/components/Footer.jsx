...
const FOOTER_CITIES = ["Delhi", "Agra", "Jaipur", "Kolkata", "Varanasi", "Hyderabad", "Hampi", "Udaipur"];
const FOOTER_CATEGORIES = ["UNESCO Sites", "Museums", "Temples", "Forts", "Palaces", "Heritage Walks", "Colonial Architecture"];
const FOOTER_TRAILS = ["Golden Triangle", "Rajasthan Royal Circuit", "Buddhist Circuit", "Temple Trail South India", "Deccan Forts Route"];
const FOOTER_RESOURCES = ["Travel Guides", "Trip Planning Blog", "FAQs", "Booking Help", "Cancellation Policy"];
    ...
    <footer className="bg-slate-900 text-slate-400">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-16 pb-12">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 lg:gap-10 mb-14">
              ...
              {["facebook", "instagram", "twitter"].map((soc) => (
                <a key={soc} href="#" aria-label={soc} className="w-8 h-8 rounded-full bg-slate-800 hover:bg-emerald-700 flex items-center justify-center transition-colors">
                  ...
                  <svg className="w-4 h-4 text-slate-400" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    {soc === "twitter" && <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>}
                  </svg>
                </a>
              ))}
            </div>
          ...
          {/* Explore Cities */}
          <div>
            <h4 className="text-white text-xs font-bold uppercase tracking-widest mb-5">Explore Cities</h4>
          ...
          {/* Explore Categories */}
          <div>
            <h4 className="text-white text-xs font-bold uppercase tracking-widest mb-5">Explore Categories</h4>
            <ul className="space-y-2.5">
          ...
          {/* Popular Trails */}
          <div>
            <h4 className="text-white text-xs font-bold uppercase tracking-widest mb-5">Popular Trails</h4>
            <ul className="space-y-2.5">
              {FOOTER_TRAILS.map((trail) => (
                <li key={trail}>
                  <a href="#" className="text-sm text-slate-500 hover:text-emerald-400 transition-colors">
                    {trail}
                  </a>
            ...
            <div className="mt-6 pt-5 border-t border-slate-800">
              ...
              <ul className="space-y-2">
                {["Privacy Policy", "Terms of Service", "Cookie Policy"].map((l) => (
                  <li key={l}><a href="#" className="text-xs text-slate-600 hover:text-slate-400 transition-colors">{l}</a></li>
                ))}
              </ul>
            </div>
          </div>
        ...
        <div className="border-t border-slate-800 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-slate-600">
            &copy; 2025 ZeQue. Crafted for heritage travelers across India.
          </p>
          <p className="text-xs text-slate-700">
            1500+ monuments &bull; 7 cities &bull; 5 curated trails
          </p>
        </div>
      </div>
    </footer>
