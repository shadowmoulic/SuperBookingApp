...
import { ChevronRight, Compass, MapPin, Clock, Globe, Star, ArrowRight } from "lucide-react";
...
export default function CityDetails() {
  ...
  // Error State
  if (error || !cityData) {
    return <ErrorScreen message={error || "City data not found"} onRetry={loadCity} />;
  }
            ...
            <div className="flex flex-col gap-4">
              {cityData.state && (
                <div className="flex items-start gap-3">
                  ...
                  <Link to={`/state/${cityData.state.toLowerCase().replace(/\s+/g, '-')}`}>
