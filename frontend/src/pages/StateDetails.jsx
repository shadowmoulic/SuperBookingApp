...
import { ChevronRight, MapPin, Clock, Globe, Star, Building2, Compass, ArrowRight, ChevronDown } from "lucide-react";
...
export default function StateDetails() {
  ...
  // Error
  if (error || !stateData) {
    return <ErrorScreen message={error || "State data not found"} onRetry={loadState} />;
  }
        ...
        <div className="absolute bottom-8 left-0 right-0 w-full px-6 md:px-12">
          <div className="max-w-[1280px] mx-auto">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-300 mb-3">
              ...
              <Link to="/states" className="hover:text-white transition-colors">States</Link>
              <ChevronRight size={13} />
          ...
          <div className="lg:col-span-2 flex flex-col justify-start">
            ...
            {stateData.description ? (
              <div className="relative">
                <div
                  className={`text-on-surface-variant font-['Inter'] text-base leading-relaxed text-justify transition-all duration-300 overflow-hidden ${isExpanded || stateData.description.length <= 250 ? "max-h-none" : "max-h-[120px] relative pb-6"
                    }`}
                >
            ...
            <div className="hidden md:flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
              <div className="relative">
                <select
                  ...
                  className="appearance-none bg-surface-container-low border border-outline-variant/50 hover:border-primary/40 rounded-xl pl-4 pr-10 py-1.5 text-xs font-semibold text-on-surface outline-none cursor-pointer focus:ring-2 focus:ring-[#006955]/20 focus:border-[#006955]"
                >
                  <option value="All Cities">All Cities</option>
                  {stateData.cities.results?.map(c => (
