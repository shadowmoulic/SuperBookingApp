...
import { usePermission, useEnterprise } from "../../hooks/usePermission";
import { LayoutDashboard, TicketPlus, LogOut, ShieldAlert, Award } from "lucide-react";
...
export default function EnterpriseLayout() {
  ...
  const enterpriseName = currentMembership?.enterprise_name || "Enterprise Portal";
  const roleName = currentMembership?.role === "booking_manager" ? "Booking Manager" : currentMembership?.role || "Member";
  ...
  const navItems = [
    ...
    {
      label: "Bulk Bookings",
      path: "/enterprise/bulk-booking",
      icon: TicketPlus,
      show: hasPermission("booking.bulk"),
    },
  ];
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-3 bg-white/5 rounded-2xl p-4 border border-white/5">
            ...
            <div className="min-w-0">
              <h2 className="font-bold text-sm text-slate-100 truncate">{enterpriseName}</h2>
          ...
          <nav className="space-y-1">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-3 mb-2">Corporate</div>
            {navItems.map((item) => {
